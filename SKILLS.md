...
description: Agent skills for working with Minions Oss-content MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-oss-content data.
---

# Minions Oss-content Agent Skills

Skills for agents operating on the `minions-oss-content` toolbox.
...
```bash
# TypeScript
pnpm add @minions-oss-content/sdk
...
The `oss-content` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-oss-content/sdk/schemas';
...
```python
from minions_oss_content.schemas import custom_types
...
- This agent only writes to `minions-oss-content` — it reads from other toolboxes but never writes to them
